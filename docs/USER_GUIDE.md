...
| `CROWDSEC_ADD_MITRE_EXT_REF_TO_SIGHTHING` | No | Boolean | Enable/disable adding external reference based on mitre technique url to the sighting related to CrowdSec. (available only if sighting is created depending on the above ``CROWDSEC_CREATE_SIGHTING` setting)<br />Default: `true` |
| `CROWDSEC_ADD_MITRE_EXT_REF_TO_ATTACK_PATTERN` | No | Boolean | Enable/disable adding external reference based on mitre technique url to attack pattern. (available only if attack pattern is created depending on the above `CROWDSEC_ATTACK_PATTERN_CREATE_FROM_MITRE` setting)<br />Default: `true` |
